...
export class KelasService {
  ...
  async ask(userId: string, question: string) {
    ...
    if (process.env.OPENAI_API_KEY) {
      const contexte = `Tu es Kelas, le coach en investissement de Kibat Invest, plateforme ivoirienne 🇨🇮.
...
Réponds en français simple, chaleureux, max 150 mots. Conseille selon ses moyens.`;
    ...
    if (q.includes('investir') || q.includes('où') || q.includes('ou ')) {
      ...
      answer = `💸 Tu peux retirer ton argent vers Orange Money CI, Wave, MTN MoMo, Moov Money ou ta carte bancaire, à partir de 1 000 FCFA. Va dans « Portefeuille » → « Retirer ».`;
    } else {
